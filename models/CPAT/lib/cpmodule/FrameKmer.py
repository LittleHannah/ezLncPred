...
def seq_generator(fastafile):
	...
	for line in ireader.reader(fastafile):
		...
		byte2 = '@'.encode(encoding='utf-8',errors='strict')	
		byte3 = '\n'.encode(encoding='utf-8',errors='strict')	
		byte4 = ' '.encode(encoding='utf-8',errors='strict')	
		str1 = line.decode(encoding='utf-8',errors='strict')
		if line.startswith((byte,byte4,byte3)):continue
		if line.startswith((byte1,byte2)):
			if tmpseq:
				yield [name,tmpseq]
				tmpseq=''
			name = line.split()[0][1:]
		elif DNA_pat.match(str1):
			tmpseq += str1 
	yield [name,tmpseq]
