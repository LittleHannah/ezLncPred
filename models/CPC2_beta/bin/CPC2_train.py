...
def __main():
	...
	Common_group.add_option("-l",dest="fasta",help="input sequence in fasta format [Required]",metavar="FILE",type="string",default=None)
	Common_group.add_option("-p",dest="pcts",help="input sequence in pct format [Required]",metavar="FILE",type="string",default=None)
	...
	lncRNA = list(SeqIO.parse(options.fasta,"fasta"))
	pcts = list(SeqIO.parse(options.pcts,"fasta"))
	print(len(lncRNA))
	print(len(pcts))
	global_y = []
	...
	#zipped = zip(all_X,global_y)
	#random.shuffle(zipped)
	#all_X,global_y = list(zip(*zipped)[0]),list(zip(*zipped)[1])
		
	
	
	if calculate_potential(all_X,strand,options.outfile,global_y):
		return 1
...
def calculate_potential(fasta,strand,outfile,label):
	'''
	...
	for seq in fasta:
		...
		ftmp_svm.write("".join(map(str,[label[i]," 1:",pep_len," 2:",fickett_score," 3:",isoelectric_point," 4:",orf_fullness]))+"\n")
		i = i+1
	ftmp_feat.close()
	ftmp_svm.close()
	#return 0
	...
	'''
	sys.stderr.write("[INFO] Training coding potential, please wait ...\n")
	...
	app_svm_train = lib_dir + "libsvm/libsvm-3.18/svm-train"
	...
	os.system(app_svm_scale + " -l 0 -u 1 -s "+ data_dir + "cpc2.range "+outfile + '.tmp.1 > ' + outfile + ".tmp.2")
	os.system(app_svm_train + " -c 16284 -g 0.125 -m 10000 -b 1 "+ outfile + ".tmp.2 "+ data_dir+'cpc2.model ')
